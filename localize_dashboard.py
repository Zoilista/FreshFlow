...
en_json_path = os.path.join("messages", "en", "dashboard.json")
...
en_data = {
  ...
  "page": {
    ...
    "empty": {
      "subtitle": "Let's get your inventory data set up.",
      "getStarted": "Get Started",
      "title": "Upload your first data file to unlock forecasts",
      "desc": "It takes less than 5 minutes. Upload a CSV or Excel file with your sales and stock data to see demand forecasts, risk analysis, and impact metrics.",
      "btn": "Upload Now →"
    },
    "banner": {
      "subtitle": "Here's your inventory status — last updated {time}.",
      "uploadBtn": "Upload New Data",
      "forecastBtn": "View Forecast →"
    },
    "urgent": {
      "title": "Requires Attention",
      "subtitle": "({count} urgent items)",
      "riskScore": "Risk score: {score}/100",
      "atRisk": "{value} at risk",
      "btn": "Create Offer →"
    },
    "stats": {
      "products": "Products Tracked",
      "productsSub": "{count} forecasts generated",
      "highRisk": "High Risk Items",
      "highRiskSubSafe": "All items safe",
      "highRiskSub": "{value} at risk",
      "foodSaved": "Food Saved (Total)",
      "foodSavedSub": "{value} t CO₂ avoided"
    },
    "actions": {
      "title": "Recommended Actions",
      "urgentTitle": "{count} high-risk items need attention",
      "urgentDesc": "{value} potential loss",
      "urgentBtn": "Create Offers",
      "forecastTitle": "Latest forecast ready",
      "forecastDesc": "Based on {file}",
      "forecastBtn": "View Forecast",
      "impactTitle": "You've prevented {value} of food waste",
      "impactDesc": "{value} revenue saved",
      "impactBtn": "See Impact Report",
      "uploadTitle": "Keep forecasts accurate",
      "uploadDesc": "Upload new data weekly for best results",
      "uploadBtn": "Upload Data"
    },
    "recent": {
      "title": "Recent Activity",
      "upload": "Data uploaded — <strong>{file}</strong>",
      "highRisk": "<strong>{count} high-risk items</strong> detected in forecast",
      "impact": "Impact updated — <strong>{value}</strong> waste prevented total",
      "today": "Today",
      "viewAll": "View all forecasts →"
    },
    "quick": {
      "title": "Quick Actions",
      "manageSurplus": "Manage Surplus"
    },
    "lastUpload": {
      "title": "Last Upload",
...
page_path = os.path.join("app", "[locale]", "(dashboard)", "dashboard", "page.tsx")
page_content = """// Dashboard Server Component — NO mock data
...
function EmptyDashboard({ displayName, t }: { displayName: string, t: any }) {
  return (
...
function AlertRow({ prediction, t, formatCurrency }: { prediction: SurplusPredictionRow, t: any, formatCurrency: any }) {
...
export default async function DashboardPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      {urgentAlerts.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-sm font-semibold text-gray-600 flex items-center gap-1.5">
            <span>⚠️</span> {t('urgent.title')} {t('urgent.subtitle', { count: urgentAlerts.length })}
          </h3>
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-semibold text-gray-600">{t('actions.title')}</h3>
        ...
        <div className="bg-white border border-gray-100 rounded-xl shadow-sm p-5">
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              ...
              <p className="flex-1 text-sm text-gray-700 leading-relaxed" dangerouslySetInnerHTML={{ __html: t.raw('recent.upload', { file: latestUpload.filename }) }} />
            ...
            {highRiskCount > 0 && (
              <div className="flex items-start gap-3">
                ...
                <p className="flex-1 text-sm text-gray-700 leading-relaxed" dangerouslySetInnerHTML={{ __html: t.raw('recent.highRisk', { count: highRiskCount }) }} />
                <span className="text-xs text-gray-400 flex-shrink-0 mt-0.5">{format.relativeTime(new Date(latestUpload.created_at))}</span>
              </div>
            )}
            {wasteKg > 0 && (
              <div className="flex items-start gap-3">
                ...
                <p className="flex-1 text-sm text-gray-700 leading-relaxed" dangerouslySetInnerHTML={{ __html: t.raw('recent.impact', { value: formatKg(wasteKg) }) }} />
